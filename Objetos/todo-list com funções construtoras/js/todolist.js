; (function () {
	...
	function Task(name, completed, createdAt, updatedAt){
        if(!name) {
            throw new Error('Task needs a required parameter.')
        }
        let _name = name
	...
	const arrInstancesTasks = arrTasks.map(function(el){
        return new Task(el.name, el.completed, el.createdAt, el.updatedAt)
    })
